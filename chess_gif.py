...
def create_gif(gif_name: str, gif_duration: int):
    ...
    imageio.mimsave(
        gif_name, images, duration=gif_duration
...
def make_args():
    ...
    ap.add_argument(
        "-d", "--duration", required=True, help="Time delay between frames of gif"
    )
    ap.add_argument("-i", "--input", required=True, help="Input PGN file")
    ap.add_argument("-o", "--output", required=True, help="Output GIF file")
    args = ap.parse_args()
    return args
